...
const COMMISSION_TYPES = [
  { value: "none", label: "Komissiya yoxdur" },
  { value: "percent", label: "Faizlə komissiya" },
...
export default function ProductsPage() {
  ...
  useEffect(() => {
    if (!editProductId || loading) return;
  ...
  }, [editProductId, loading, products, productRequirements, editingProductId]);
  ...
  const renderProductCommission = (item) => {
    if (!item.has_commission || item.commission_type === "none") {
      return "Yoxdur";
    }
  ...
  const saveProduct = async (e) => {
    ...
    const selectedCommissionType = productForm.commission_type || "none";

    if (selectedCommissionType === "percent" && productForm.commission_percent === "") {
      setMessage("Komissiya faizi qeyd edilməlidir.");
      return;
    }

    if (selectedCommissionType === "fixed" && productForm.commission_amount === "") {
      setMessage("Komissiya məbləği qeyd edilməlidir.");
      return;
    }
    ...
    const payload = {
      ...
      has_commission: selectedCommissionType !== "none",
      commission_type: selectedCommissionType,
      commission_percent:
        selectedCommissionType === "percent" ? Number(productForm.commission_percent) : null,
      commission_amount:
        selectedCommissionType === "fixed" ? Number(productForm.commission_amount) : 0,
  ...
  const startEditProduct = (item) => {
    ...
    const legacyFixedCommission =
      item.has_commission === true &&
      (!item.commission_type || item.commission_type === "none") &&
      Number(item.commission_amount || 0) > 0;
    const nextCommissionType =
      item.commission_type === "percent"
        ? "percent"
        : item.commission_type === "fixed" || legacyFixedCommission
        ? "fixed"
        : "none";
    ...
    setProductForm({
      ...
      has_commission: nextCommissionType !== "none",
      commission_type: nextCommissionType,
      commission_percent:
        nextCommissionType === "percent" ? item.commission_percent ?? "" : "",
      commission_amount:
        nextCommissionType === "fixed" ? item.commission_amount ?? "" : "",
  ...
  return (
    <div>
      <div style={styles.header}>
        <div>
          <h1 style={styles.title}>Məhsul ayarları</h1>
          <p style={styles.subtitle}>
            Kredit formaları, məhsul yaratma, faiz, komissiya və dinamik şərtləri buradan idarə et.
          </p>
        </div>
      </div>
        ...
        <section style={styles.panel}>
          ...
          <form onSubmit={saveProduct}>
            ...
            <div style={styles.formGrid}>
              ...
              <div>
                <label style={styles.label}>Komissiya tipi</label>
                <select
                  style={styles.select}
              ...
              {productForm.commission_type === "percent" ? (
                <div>
                  <label style={styles.label}>Komissiya faizi (%)</label>
                  <input
                    type="number"
                    step="0.01"
                    style={styles.input}
              ...
              {productForm.commission_type === "fixed" ? (
                <div>
